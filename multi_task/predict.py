...
class TransformerPredictor:
    ...
    def predict(self):
        ...
        while next(self.config):
            ...
            trainer = TransformerTrainer(model=model,
                                         ...
                                         map_location=map_location)
            # train
            fig, loss, metric_output1, metric_output2 = trainer.train()
            ...
            if trainer.datasets2['test']:
                loss, metric_output1, metric_output2, report1, report2 = trainer.test('tmp-checkpoint.pth')
                #loss, metric_output1, metric_output2, report = trainer.test('tmp-checkpoint.pth')
                self.clear()

            if self.config.process['is_first']:
                s = self._get_output_prefix(loss, metric_output1, metric_output2)
                ...
                if metric_output1 > best_metric1[0] or self.config.iteration == 1:
                    best_metric1 = (metric_output1, model_out)
                    logging.info(f"!! New highest metric 1(Pearson"
                                 f" {best_metric1[0]:.4f}): {best_metric1[1]}")

                if metric_output2 > best_metric2[0] or self.config.iteration == 1:
                    best_metric2 = (metric_output2, model_out)
                    logging.info(f"!! New highest metric 2(F1"
                                 f" {best_metric2[0]:.4f}): {best_metric2[1]}")
                ...
                if report1:
                    with self.output_p.joinpath('report1.txt').open('w', encoding='utf-8') as fhout:
                        fhout.write(report1)
                if report2:
                    with self.output_p.joinpath('report2.txt').open('w', encoding='utf-8') as fhout:
                        fhout.write(report2)
        ...
        logging.info(f"Model with smallest loss ({self.config.training['metric_name']} {best_loss[0]:.4f})"
                     f": {best_loss[1]}")
        logging.info(f"Model with highest metric 1(Pearson {best_metric1[0]:.4f}):"
                     f" {best_metric1[1]}")
        logging.info(f"Model with highest metric 2(F1 {best_metric2[0]:.4f}):"
                     f" {best_metric2[1]}")
    ...
    def _get_output_prefix(self, loss, metric_output1, metric_output2):
        ...
        # results
        s = f"loss{loss:.2f}-Pearson{metric_output1:.2f}-F1{metric_output2:.2f}-"
        # optimizer
