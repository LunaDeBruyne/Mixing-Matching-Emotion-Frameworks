...
class TransformerClassifier(nn.Module):
    ...
    def __init__(self):
        ...
        self.classifier1 = nn.Linear(self.mopts['pre_classifier'] if self.mopts['pre_classifier'] else pooled_dim,
                                    self.config.training['num_labels1'])
        self.classifier2 = nn.Linear(self.mopts['pre_classifier'] if self.mopts['pre_classifier'] else pooled_dim,
                                    self.config.training['num_labels2'])
    ...
    def forward(self, t, input_ids,
        ...
        if t == 1:
            clf = self.classifier1(output)
        elif t == 2:
            clf = self.classifier2(output)
...
class TransformerTokenizer:
    ...
    def encode_batch_plus(self,
        ...
        for first_sent, second_sent in zip(batch, batch_pair):
            ...
            if self.lexicon == 'True':
                feat_dict = load_object('feat_dict_subtitles_new.pkl')
                dict2 = {}
